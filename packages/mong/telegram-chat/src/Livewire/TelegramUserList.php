...
class TelegramUserList extends Component
{
    ...
    protected $listeners = [
        ...
        'echo:telegram-messages,.message-received' => 'handleNewMessage',
    ];
    ...
    public function refreshUsers()
    {
        ...
        $this->users = $query->latest()
            ->get()
            ->map(function ($user) {
                $unreadMessages = TelegramMessage::where('chat_id', $user->id)
                    ->where('is_read', false)
                    ->where('direction', 'in') // Only count incoming messages
                    ->get();

                $user->unread_count = $unreadMessages->count();
                $user->last_message_time = $user->lastMessage?->created_at;
                $user->last_message_preview = $user->lastMessage ? Str::limit($user->lastMessage->text, 30) : null;
    ...
    public function selectUser($userId)
    {
        ...
        TelegramMessage::where('chat_id', $userId)
            ->where('is_read', false)
    ...
    {
        return view('telegram-chat::livewire.telegram-user-list');
    }
}
